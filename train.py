...
def run(rank, n_gpus, hps):
    ...
    # ✅ use config num_workers if provided; keep a safe fallback
    cfg_num_workers = getattr(hps.train, "num_workers", None)
    ...
    try:
        ...
        m = re.search(r"D_(\d+)\.pth$", base)
        if m:
            global_step = int(m.group(1)) + 1
        else:
            m = re.search(r"_step(\d+)\.pth$", base)
            if m:
                global_step = int(m.group(1)) + 1
            else:
                # 最後の手段：epochから推定（安全側）
    ...
    for epoch in range(epoch_str, hps.train.epochs + 1):
        ...
        # ✅ epoch-based checkpoint (e.g., every 10 epochs)
        if rank == 0:
            save_every_epochs = int(getattr(hps.train, "save_every_epochs", 10))
            if save_every_epochs > 0 and (epoch % save_every_epochs) == 0:
                logger.info(f"[EPOCH-SAVE] epoch={epoch} global_step={global_step} model_dir={hps.model_dir}")
                utils.save_checkpoint(
                    net_g,
                    optim_g,
                    hps.train.learning_rate,
                    epoch,
                    os.path.join(hps.model_dir, f"G_epoch{epoch}_step{global_step}.pth"),
                )
                utils.save_checkpoint(
                    net_d,
                    optim_d,
                    hps.train.learning_rate,
                    epoch,
                    os.path.join(hps.model_dir, f"D_epoch{epoch}_step{global_step}.pth"),
                )

                # keep only newest checkpoints if supported by utils
                keep_ckpts = int(getattr(hps.train, "keep_ckpts", 0))
                if keep_ckpts > 0 and hasattr(utils, "clean_checkpoints"):
...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
    ...
    # ✅ unified save interval: prefer save_interval if present, else eval_interval
    save_interval = int(getattr(hps.train, "save_interval", getattr(hps.train, "eval_interval", 200)))
    keep_ckpts = int(getattr(hps.train, "keep_ckpts", 0))
    do_eval_on_save = True  # eval_loader exists only on rank 0 anyway
    ...
    for batch_idx, items in enumerate(train_loader):
        ...
        if rank == 0:
            ...
            # ✅ Save/Eval: every save_interval steps
            if save_interval > 0 and (global_step % save_interval) == 0:
                logger.info(f"[EVAL/SAVE] epoch={epoch} global_step={global_step} model_dir={hps.model_dir}")
                if do_eval_on_save and eval_loader is not None:
